...
export const Posts = () => {
    ...
    return (
        <ContentLayout title="Posts">
            ...
            <div className="mt-4">
                { postsQuery.data.posts.map((post: Post, index: number) => <PostCard key={ index } post={ post } />) }
            </div>
            <div className="flex flex-row">
                <div className="flex-1 text-left">
                    <Link to={`./?page=${pagination.currentPage - 1}`} className="text-gray-900 font-bold text-xl mb-2">
                        <button onClick={ () => { setPage((page: number) => page - 1); scrollToTop(); } } disabled={ !pagination.prevPageUrl }
                ...
                <div className="flex-1 text-right">
                    <Link to={`./?page=${pagination.currentPage + 1}`} className="text-gray-900 font-bold text-xl mb-2">
                        <button onClick={ () => { setPage((page: number) => page + 1); scrollToTop(); } } disabled={ !pagination.nextPageUrl }
                                className="text-sm bg-gray-200 hover:bg-gray-300 text-gray-800 font-semibold py-2 px-4 rounded-l disabled:opacity-50 disabled:cursor-not-allowed"
                        >
                            Next
                        </button>
